...
local RunBot = io.open("RATER", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/RATER
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr RATER.lua
wget "https://raw.githubusercontent.com/RATER-AD/RATER/main/RATER.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./RATER.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S RATER -X kill
screen -S RATER ./RATER
done
]])
RunTs:close()
end
